...
export const PlaygroundEditor = ({
  ...
  // FIX: handleEditorDidMount runs exactly once (Monaco's onMount fires only
  // on first mount). Any prop captured directly inside its listeners would be
  // frozen at that first render forever. These refs are kept in sync every
  // render so the long-lived listeners always read the latest values.
  ...
  }, [onTriggerSuggestion, onRejectSuggestion, suggestionLoading])
  ...
  const createInlineCompletionProvider = useCallback(
    (monaco: Monaco) => {
      return {
        ...
        // Monaco renamed this cleanup hook from freeInlineCompletions to
        // disposeInlineCompletions (now takes a `reason` param too). The
        // old name silently doesn't exist anymore on 0.55.1+, so Monaco
        // throws "this.provider.disposeInlineCompletions is not a
        // function" the moment it tries to clean up returned completions.
  ...
  useEffect(() => {
    ...
    if (suggestion && suggestionPosition) {
      ...
      // No suggestion (e.g. AI was just toggled off) — actively hide
      // whatever ghost text Monaco is currently rendering instead of
      // leaving it on screen until the next cursor move or keystroke.
  ...
  const handleEditorDidMount = (editor: any, monaco: Monaco) => {
    ...
    // NOTE: editor.addCommand() returns a string command ID, not an
    // IDisposable — there is nothing to "dispose" here. (This guard is
    // effectively dead code anyway since handleEditorDidMount only runs
    // once per editor mount, but removing the bad .dispose() call so it
    // can't throw if that ever changes.)
    ...
    editor.addCommand(monaco.KeyCode.Escape, () => {
      ...
      if (currentSuggestionRef.current) {
        onRejectSuggestionRef.current(editor)
        clearCurrentSuggestion()
      }
    })
    ...
    editor.onDidChangeCursorPosition((e: any) => {
      ...
      if (!currentSuggestionRef.current && !suggestionLoadingRef.current) {
    ...
    editor.onDidChangeModelContent((e: any) => {
      ...
      if (e.changes.length > 0 && !suggestionAcceptedRef.current) {
        ...
        if (
          ...
          setTimeout(() => {
            if (editorRef.current && !currentSuggestionRef.current && !suggestionLoadingRef.current) {
              onTriggerSuggestionRef.current("completion", editor)
            }
  ...
  useEffect(() => {
    return () => {
      ...
      if (tabCommandRef.current) {
        // addCommand returns a string command ID, not an IDisposable —
        // nothing to dispose, just clear the ref.
